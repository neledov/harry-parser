...
function loadRequestDetail(index) {
    ...
    fetch(requestUrl)
        ...
        .then(data => {
            ...
            } else if (mimeType.includes('html') || mimeType.includes('markup')) { // Prism.js uses 'markup'
            ...
            if (data.request && data.request.postData && data.request.postData.text) {
                ...
                postDataContent = `
                    <div class="section">
                        ...
                        <button class="copy-button" data-text="${encodeURIComponent(postDataText)}" aria-label="Copy Post Data">Copy</button>
                    </div>
                `;
            ...
            if (response.content && response.content.text) {
                ...
                responseContent = `
                    <div class="section">
                        ...
                        <button class="copy-button" data-text="${encodeURIComponent(responseText)}" aria-label="Copy Response Content">Copy</button>
                    </div>
                `;
            ...
            if (data.request && data.request.headers && Array.isArray(data.request.headers)) {
                data.request.headers.forEach(header => {
                    requestHeadersHtml += `<li><strong>${escapeHTML(header.name)}:</strong> ${escapeHTML(header.value)}</li>`;
                });
            ...
            if (response.headers && Array.isArray(response.headers)) {
                response.headers.forEach(header => {
                    responseHeadersHtml += `<li><strong>${escapeHTML(header.name)}:</strong> ${escapeHTML(header.value)}</li>`;
                });
            ...
            if (data.request && data.request.queryString && Array.isArray(data.request.queryString) && data.request.queryString.length > 0) {
                data.request.queryString.forEach(param => {
                    queryParamsHtml += `<li><strong>${escapeHTML(param.name)}:</strong> ${escapeHTML(param.value)}</li>`;
                });
            ...
            if (data.timings && typeof data.timings === 'object') {
                for (const [key, value] of Object.entries(data.timings)) {
                    timingsHtml += `<li><strong>${escapeHTML(key)}:</strong> ${escapeHTML(value)}</li>`;
                }
                console.log('Timings generated.');
            } else {
                timingsHtml += `<li>No timing information available.</li>`;
                console.log('No Timing information available.');
            }
            timingsHtml += '</ul>';

            // Safely handle response status and statusText
            const status = (response.status !== undefined && response.status !== null) ? response.status : 'N/A';
            const statusText = (response.statusText) ? response.statusText : 'No Status Text';
            ...
            const curlButtonHtml = `
                <div class="section">
                    <button id="copy-curl" class="curl-button" aria-label="Copy cURL Command">cURL</button>
            ...
            let html = `
                <div class="section">
                    ...
                    <p><strong>Method:</strong> ${escapeHTML(data.request.method || 'N/A')}</p>
                    <p><strong>URL:</strong> <a href="${escapeHTML(data.request.url || '#')}" target="_blank">${escapeHTML(data.request.url || 'N/A')}</a></p>
                </div>
                ...
                <div class="section">
                    ...
                    <p><strong>Status:</strong> ${escapeHTML(status)} ${escapeHTML(statusText)}</p>
                </div>
            ...
            // hideSpinner('request-detail'); // Removed to prevent clearing innerHTML
            ...
            if (curlButton) {
                curlButton.addEventListener('click', () => {
                    copyToClipboard(encodeURIComponent(curlCommand));
                });
            }
        })
        .catch(error => {
            ...
            const detailDiv = document.getElementById('request-detail');
            detailDiv.innerHTML = '<p class="error">Error loading request details.</p>';
            hideSpinner('request-detail');
        });
